...
origMRdata = load_nii(['example/sourcedata/' sub '/anat/' sub '_T2w_space-T1w*.nii.gz']);
MRdata = zeros(sz);
MRdata(:) = double(origMRdata.img(cropping));
origMRdata = [];
...
%%
load('misc/avgBorders_NeuroImage2017study.mat')
...
for n = 1:nSubPlots
    ...
    axis equal; h = colorbar; ylabel(h, 'T2w intensity');
